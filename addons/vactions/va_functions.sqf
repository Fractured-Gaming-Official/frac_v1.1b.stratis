...
va_outside_owner_add_actions = {
  ...
  {if (true) then {
    ...
    def(_action_id);
    _action_id = _player addaction [format["<img image='addons\vactions\icons\pull.paa'/> Pull %1", (name _member)], {_this call va_pull_player_action;}, [_player, _vehicle, _member],2,false,false,"",
...
va_outside_add_actions = {
  ...
  _action_id = _player addaction [format["<img image='addons\vactions\icons\flip.paa'/> Unflip %1", _display_name], {_this call va_unflip_action;}, [_player, _vehicle],-7,false,false,"",
  format["([objectFromNetId %1] call va_unflip_action_available)",str(netId _vehicle)]];
  va_outside_actions = va_outside_actions + [_action_id];
  

  //Add view vehicle information action
  _action_id = player addaction [format["<img image='addons\vactions\icons\info.paa'/> %1 info", _display_name], {_this call va_information_action;}, [_player, _vehicle],-6,false,false,"",
...
va_inside_add_actions = {
  ...
  //Add vehicle lock action
  _action_id = player addaction [format["<img image='addons\vactions\icons\lock.paa'/> Lock %1", _display_name], {_this call va_lock_action;}, [_player, _vehicle],-2,false,false,"",
  format["([objectFromNetId %1, objectFromNetId %2] call va_lock_action_available)", str(netId _player), str(netId _vehicle)]];
  va_inside_actions = va_inside_actions + [_action_id];
  
  //Add vehicle unlock action
  _action_id = player addaction [format["<img image='addons\vactions\icons\key.paa'/> Unlock %1", _display_name], {_this call va_unlock_action;}, [_player, _vehicle],-2,false,false,"",
  format["([objectFromNetId %1, objectFromNetId %2] call va_unlock_action_available)", str(netId _player), str(netId _vehicle)]];
  va_inside_actions = va_inside_actions + [_action_id];
};
